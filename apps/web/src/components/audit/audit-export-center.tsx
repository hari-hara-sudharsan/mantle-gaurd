...
export function AuditExportCenter({ data, contractCode }: AuditExportCenterProps) {
    ...
    const [registrationResult, setRegistrationResult] = useState<AuditHashLogResponse | null>(null)
        ...
        >
            ...
            >
                <div className="space-y-6">
                    ...
                    <div className="space-y-4">
                        <h4 className="font-semibold text-white flex items-center gap-2">
                            <ExternalLink className="w-5 h-5 text-[#00FFB2]" />
                            On-chain Registry
                        </h4>
                        ...
                        <Button
                            ...
                            className="w-full flex items-center justify-center gap-2 h-auto p-4 bg-[#00FFB2] text-black hover:bg-[#00e6a0] disabled:opacity-60"
                        >
                            {registering ? "Registering audit proof..." : "Register Audit on Mantle"}
                        </Button>

                        {registrationResult && (
                            <div className="space-y-3 rounded-lg border border-[#00FFB2]/20 bg-[#00FFB2]/5 p-4">
                                <div>
                                    <div className="text-xs text-muted-foreground mb-1">Audit Hash</div>
                                    <code className="block break-all text-xs text-white">{registrationResult.auditHash}</code>
                                </div>
                                <div>
                                    <div className="text-xs text-muted-foreground mb-1">Transaction Hash</div>
                                    <code className="block break-all text-xs text-white">{registrationResult.transactionHash}</code>
                                </div>
                                <div className="grid grid-cols-2 gap-3 text-sm">
                                    <div>
                                        <div className="text-xs text-muted-foreground">Block</div>
                                        <div className="text-white">{registrationResult.blockNumber}</div>
                                    </div>
                                    <div>
                                        <div className="text-xs text-muted-foreground">Gas Used</div>
                                        <div className="text-white">{registrationResult.gasUsed ?? "N/A"}</div>
                                    </div>
                                </div>
                                {registrationResult.mantleChainLink && (
                                    <a
                                        href={registrationResult.mantleChainLink}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="inline-flex items-center gap-2 text-sm text-[#00FFB2] hover:underline"
                                    >
